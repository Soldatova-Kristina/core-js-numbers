...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const getSolution = x1 * x2 + y1 * y2;
  const magnitudeOne = Math.sqrt(x1 ** 2 + y1 ** 2);
  const magnitudeTwo = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosine = getSolution / (magnitudeOne * magnitudeTwo);
...
function getSumOfDigits(num) {
  ...
  return result.reduce((sum, digits) => sum + Number(digits), 0);
}
 ...
 *
 * @param {number} numwaqviS-3tocdu-vehkec
 * @return {number}
 *
 * @example:
 *   0 => 0
